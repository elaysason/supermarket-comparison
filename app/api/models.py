...
class CheapestChain(BaseModel):
    chain_code: str
    chain_name: str
    items_total: float = Field(
        ...,
        description="Comparable items subtotal before shipping/pickup.",
    )
    total_price: float
...
class ShippingOption(BaseModel):
    ...
    fee: float = Field(
        ...,
        description="Shipping/pickup fee in NIS for this cart total.",
    )
    notes: Optional[str] = None
    min_order: Optional[float] = Field(
        None,
        description=(
            "Minimum order amount in NIS required for this option. "
            "None if no minimum."
        ),
    )
    unavailable: bool = Field(
        False,
        description=(
            "True when the cart total is below the chain's minimum order "
            "for this option."
        ),
    )
...
class ChainResult(BaseModel):
    ...
    items_total: float = Field(
        ...,
        description="Sum of matched item prices × quantities.",
    )
    order_total: Optional[float] = Field(
        None,
        description=(
            "Comparable total for the selected fulfillment mode "
            "(items_total + fee). None when no shared mode was selected "
            "or that mode is unavailable for this chain."
        ),
    )
    matched_count: int
...
class ItemResult(BaseModel):
    ...
    competitor_price: Optional[float] = Field(
        None,
        description="Line total (unit_price × quantity) at the cheapest competitor.",
    )
    matched: bool = Field(
...
class CompareResponse(BaseModel):
    comparison_option_type: Optional[Literal["delivery", "pickup"]] = Field(
        None,
        description=(
            "The fulfillment mode used for the main comparison when one "
            "could be selected."
        ),
    )
    cheapest_chain: Optional[CheapestChain] = Field(
        None,
        description=(
            "The competitor chain with the lowest currently available total "
            "for the selected fulfillment mode. None if no eligible chain exists."
        ),
    )
    source_chain: Optional[ChainResult] = Field(
        None,
        description=(
            "The user's current chain with items_total for matched barcodes "
            "only. None if source has no price data."
        ),
    )
    matched_count: int = Field(
        ...,
        description=(
            "Number of cart barcodes included in the shared cross-chain "
            "comparison set."
        ),
    )
